...
| `issue_category` | — | |
| `issue_description` | — | Maps to the `reported_symptoms` column |
...
Only these fields can be updated: `issue_category`, `issue_description` (→ `reported_symptoms`),
`priority`, `assigned_tech_id`, `findings_summary`. **Status cannot be changed via `PATCH`** — use
the transition endpoint. A no-op (no field actually changed) returns `200` and writes nothing.
...
validation as the staff app** (state graph + role permissions + exit gates + payment-completion
gate). An API `write` key acts with **OWNER-equivalent** transition permissions.
